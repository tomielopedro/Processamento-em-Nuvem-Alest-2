...
class TaskScheduler:
    ...
    def calcular_tempo_ocioso(self, tempo_total_escalonado: int) -> int:
        tarefas = self.coletar_tarefas()
        soma_tempos = sum(t.task_time for t in tarefas)
        return (tempo_total_escalonado * self.num_proc) - soma_tempos

    def compara_politicas(self):
        self.politica = 'MIN'
        min_tempo, min_ordem_execucao = self.escalonar()
        min_tempo_ocioso = self.calcular_tempo_ocioso(min_tempo)

        self.politica = 'MAX'
        max_tempo, max_ordem_execucao = self.escalonar()
        max_tempo_ocioso = self.calcular_tempo_ocioso(max_tempo)
        ...
        info = {
            ...
            "tempo_ocioso_max": max_tempo_ocioso,
            "politica_min": min_tempo,
            "tempo_ocioso_min": min_tempo_ocioso,
    ...
    def to_dict(self, tempo) -> dict:
        ...
        media = soma_tempos / qtd if qtd else 0
        proc_tarefas = round(len(tarefas) / self.num_proc, 2)
        tempo_ocioso = self.calcular_tempo_ocioso(tempo)
        ...
        relatorio_dict = {
            ...
            "tempo_total_ocioso": tempo_ocioso,
            "proporcao_proc_tarefas": proc_tarefas,
            "media_tempo_por_tarefa": round(media, 2),
        }
    ...
    def simular_e_plotar_ociosidade(self):
        tarefas = self.coletar_tarefas()
        if not tarefas:
            print("Nenhuma tarefa para simular.")
            return

        num_max_proc = len(tarefas)
        processadores = range(1, num_max_proc + 1)
        ociosidade_min = []
        ociosidade_max = []
        ...
        for p in processadores:
            ...
            # Simula com política MIN
            self.politica = 'MIN'
            tempo_min, _ = self.escalonar()
            ociosidade_min.append(self.calcular_tempo_ocioso(tempo_min))

            # Simula com política MAX
            self.politica = 'MAX'
            tempo_max, _ = self.escalonar()
            ociosidade_max.append(self.calcular_tempo_ocioso(tempo_max))

        plt.figure(figsize=(12, 8))
        plt.plot(processadores, ociosidade_min, marker='o', label='Política MIN (Menor Tempo Ocioso)')
        plt.plot(processadores, ociosidade_max, marker='x', label='Política MAX (Menor Tempo Ocioso)')

        plt.title('Tempo de Ociosidade Total vs. Número de Processadores')
        plt.xlabel('Número de Processadores')
        plt.ylabel('Tempo de Ociosidade Total')
        plt.legend()
        plt.grid(True)
        plt.xticks(processadores)
        plt.show()
...
if __name__ == "__main__":
    ...
    comparativo = scheduler.compara_politicas()
    print("Relatório de Comparação de Políticas:")
